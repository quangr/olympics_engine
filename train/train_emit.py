...
class CnnPolicy(nn.Module):
    ...
    def forward(self,input:torch.Tensor, state:torch.Tensor=None, info:Dict={})->torch.Tensor:
        return self.locmodel(torch.tensor(input,dtype=torch.float32,device=device)),state
    def close(self)->None:
        return
...
train_collector = ts.data.Collector(policy, train_envs, ts.data.VectorReplayBuffer(200000, 500), exploration_noise=True,preprocess_fn=test_processor.preprocess_fn)
...
trainer =ts.trainer.OnpolicyTrainer(
    ...
    max_epoch=500, step_per_epoch=500, step_per_collect=100,
    repeat_per_collect=2, episode_per_test=10, batch_size=1024,
