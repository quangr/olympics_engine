...
def workspace():
    ...
    mypy_integration_deps(mypy_requirements_file = "@olympics_engine//third_party/mypy:mypy_version.txt")

    if "mypy_integration_config" not in native.existing_rules().keys():
        mypy_configuration("@olympics_engine//third_party/mypy:mypy.ini")
